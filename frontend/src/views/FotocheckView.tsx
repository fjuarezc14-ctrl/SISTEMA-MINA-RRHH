...
import { BadgeCheck, Printer, QrCode, CheckCircle, Shield, HardHat, Calendar } from 'lucide-react';
...
export const FotocheckView: React.FC<FotocheckProps> = ({ postulantes, fotochecks, onImprimir }) => {
  // Filtrar los que están en fase FOTOCHECK o APROBADO_TOTAL
  const candidatosAptos = postulantes.filter(
    (p) => p.fase_actual === 'FOTOCHECK' || p.estado_global === 'APROBADO_TOTAL'
  );
  ...
  return (
    <div className="space-y-6">
      <div className="bg-slate-800 rounded-2xl border border-slate-700 p-8 flex flex-col items-center justify-center text-center shadow-2xl">
        ...
        <h3 className="text-2xl font-black text-white tracking-tight">Centro de Impresión y Habilitación de Garita</h3>
        <p className="text-slate-400 text-sm max-w-md mt-1 mb-6">
          Los trabajadores mostrados a continuación han completado satisfactoriamente las 5 fases de compliance minero.
        </p>
        ...
        <div className="bg-gradient-to-b from-slate-950 to-slate-900 border-2 border-slate-700 rounded-2xl p-6 w-80 shadow-2xl text-left relative overflow-hidden mb-6">
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-2.5 text-[10px] space-y-1 text-slate-400">
            <div className="flex justify-between">
              <span>Empresa:</span>
              <span className="text-white font-medium">{selectedCandidate.empresa_nombre || 'Servicios XYZ'}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              <span>Vigencia SCTR:</span>
              <span className="text-emerald-400 font-semibold">1 Año (Activo)</span>
            </div>
          </div>
        ...
        {impresoSuccess && (
          <div className="mb-4 flex items-center gap-2 bg-emerald-950/60 border border-emerald-500/40 text-emerald-300 text-xs px-4 py-2 rounded-xl">
            <CheckCircle className="w-4 h-4" /> Fotocheck enviado a cola de impresión y habilitado en garita.
          </div>
        )}
        ...
        >
          ...
          {loading ? 'Procesando...' : `Imprimir Fotocheck de ${selectedCandidate.apellidos}, ${selectedCandidate.nombres}`}
        </button>
      </div>
    </div>
  );
